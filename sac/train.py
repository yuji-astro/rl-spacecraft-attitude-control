...
def train():
    ...
    #logger
    wandb.init(project='Satellite-continuous-SAC',
        config={
        "batch_size": 128,
        "critic_lr": 1e-3,
        "actor_lr": 1e-4,
    ...
    self_entropy_tuning = config.self_entropy_tuning
    ...
    agent = SACAgent(env, gamma, tau, alpha, buffer_maxlen, critic_lr, actor_lr, alpha_lr, True, policy_type, self_entropy_tuning,
    ...
    if not os.path.isdir("models"):
        os.mkdir("models")
    torch.save(agent, curr_dir + "/models/spacecraft_control_sac.pkl")
...
def evaluate():
    ...
    agent = torch.load(curr_dir + "/models/spacecraft_control_sac.pkl",map_location='cpu')
